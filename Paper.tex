...
%With regards to the goal of creating explainable systems, however, unsound rationales could create irrational explanations which would be problematic for the XAI movement.
...
%XAI aims to create explainable models that are able to explicitly describe the inner workings of a ``black box'' machine learning system, and thus provide the essential explanations to their users. 
...
%For instance, a machine learning algorithm may be able to easily learn two conditions separately, but have difficulty when attempting to learn both at the same time. 
...
For defining conditions that generate datasets, we use %The type of functions that neural networks and other machine learning algorithms have been shown to struggle with in previous research can be generalized into 
...
%, showing the robustness of machine learning results. 
...
%When an instance of this dataset has a true output, and one of its variables is inverted, the total number of 1's will no longer be equal to $m$, and therefore the output will be incorrect. If an instance in the exact dataset has an output that is false, however, the noise can do two things: the number of 1's might be equal to $m$, thus creating a true output, or (the more likely alternative) the number of 1's may still not be equal to $m$, and the output is still false. This effect can be seen in Figure \ref{fig:SBF_EU}, as the systems are only able to correctly classify the incorrect instances, thus achieving an accuracy of roughly 50\%.  
...
In most of the experiments, the performances of the neural network and the decision trees were quite similar. %When considering the fact that decision trees are transparent and much more easy to comprehend, they would appear to be superior to the ``black box''  neural network. 
