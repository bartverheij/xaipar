...
Many of the successful modern machine learning approaches can be described as ``black box'' systems; these systems perform well, but are unable to explain the reasoning behind their decisions. The emerging sub-field of Explainable Artificial Intelligence (XAI) aims to create systems that are able to explain to their users why they made a particular decision. Using artificial datasets whose internal structure is known beforehand, this study shows that the reasoning of neural networks and decisions trees that perform well are not necessarily sound. Furthermore, when multiple conditions define a dataset, systems can perform well on the combined problem yet not learn each of the individual conditions that make up the combined problem. Instead, a confounding structure within the data is learnt that still allows making the correct decisions. With regards to the goal of creating explainable systems, however, unsound rationales could create irrational explanations which would be problematic for the XAI movement.
\end{abstract}
...
\subsection{Datasets}
%%%BV VERHELDEREN
For our experiment, a parametrized series of datasets is generated based on conditions defined in terms of symmetrical Boolean functions. Each instance in the dataset has an output value, which is true if and only if all of its conditions are true as well. This set of conditions can include the M-out-of-N function, the exact value function and the parity function. Each condition also has a set of parameters. As described earlier, parameter $n$ denotes the number of variables of the condition and parameter $m$ is used in M-out-of-N functions and exact value functions to determine the value of the output. The total number of variables of each instances in the datasets is therefore based on the number of variables of each of the conditions. Table \ref{tbl:instance_example} shows two example instances of such a dataset. The instance of this dataset consist of two conditions: the parity condition with 2 variables (in red) and the M-out-of-N condition with 3 variables (in blue), where parameter $m$ is set to 2. In the first instance, the parity condition is false and the M-out-of-N condition is true. The output is therefore false, because all conditions  of an instance need to be true in order for the output to be true. In the second instance, the output is true, as both the parity condition (red) and the M-out-of-N condition (blue) are true.

\begin{table}[t]
\caption{Two example instances of a dataset with 2 parity variables (red) and 3 M-out-of-N variables (blue), where $m$=2.}
\centering
...
Even under extreme levels of noise, the systems are still able to learn the M-out-of-N and exact value function quite effectively, showing the robustness of machine learning results. 

%%%CS Leave this out entirely, or move it to discussion?
\iffalse
The M-out-of-N function is most robust to noise, as seen in Figure \ref{fig:SBF_MU}. This is because inverting one variable of the input vector only changes the output in two cases. First of all, if the total number of 1's is equal to $m$ and the output is true, changing a 1 to a 0 due to noise should change the output to false. Secondly, if the total number of 1's is equal to $m-1$ and the output is false, and the noise changes a variable from 0 to 1, the output should be true.  

The accuracies on the exact function drop faster with more noise, as seen in Figure \ref{fig:SBF_EU}. When an instance of this dataset has a true output, and one of its variables is inverted, the total number of 1's will no longer be equal to $m$, and therefore the output will be incorrect. If an instance in the exact dataset has an output that is false, however, the noise can do two things: the number of 1's might be equal to $m$, thus creating a true output, or (the more likely alternative) the number of 1's may still not be equal to $m$, and the output is still false. This effect can be seen in Figure \ref{fig:SBF_EU}, as the systems are only able to correctly classify the incorrect instances, thus achieving an accuracy of roughly 50\%.  

The parity function is a different case, as its output is dependent on whether the amount of 1's in the input vector is odd or even. By adding a noisy variable to an instance of the parity function, the amount of 1's will always change from odd to even, or even to odd. This means that every instance of a pairty dataset that has a noisy variable automatically has the wrong output. This is why the accuracies on the parity function drops more significantly than on the other functions (to 0\% in the case of the decision tree in Figure \ref{fig:SBF_PU}). 
\fi

%%%BV WHAT DO YOU WANT TO SAY HERE? CONNECTION TO DATA SHOWN?
